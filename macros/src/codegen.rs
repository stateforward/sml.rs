...
use crate::event_codegen::{add_spec, names_table, visit_calls, EventSpecs};
...
pub fn generate_code(sm: &ParsedStateMachine) -> proc_macro2::TokenStream {
    ...
    let event_idents = event_list.clone();
    ...
    let event_list: Vec<_> = event_list
        ...
        .collect();
    let event_name_arms = event_idents.iter().map(|value| {
        if sm.event_data.data_types.contains_key(&value.to_string()) {
    ...
    for (state_name, event_mappings) in &sm.states_events_mapping {
        ...
        for mapping in event_mappings.values() {
            ...
            if event_type.is_some_and(|event_type| sm.type_uses_event_generics(event_type)) {
                continue;
            }
            add_spec(
                &mut event_specs,
        ...
        let state_pattern = if sm.state_data.data_types.contains_key(state_name) {
            quote! { #states_type_name::#state(..) }
        } else {
            quote! { #states_type_name::#state }
        };
        let visits = visit_calls(&state_specs);
        current_event_query_arms.push(quote! {
            #state_pattern => { #visits }
        });
    }
    ...
    let event_names: Vec<Vec<_>> = transitions
        .values()
        .map(|event_mappings| {
            event_mappings
                .values()
                .map(|mapping| {
                    let name = mapping.event.to_string();
                    quote! { #name }
                })
                .collect()
        })
                            ...
                            guard.iter().zip(
                                ...
                                let process_code = process_events.iter().map(|event| {
                                    if async_queue {
                                        ...
                                        quote! { let _ = self.__sml_process_event_unlocked(#event)#process_await?; }
                                    }
                                });
                                ...
                                let transition = if *internal_transition {
                                    ...
                                    quote!{
                                            ...
                                            Self::__sml_log_state_change(&mut self.policy, #in_state_string, #out_state_string);
                                ...
                                let candidate_dispatch = if matches!(*event_kind, EventKind::Normal | EventKind::Unexpected) {
                                    quote! {
                                        ...
                                        quote! {
                                            {
                                                ...
                                                Self::__sml_log_guard(&mut self.policy, stringify!(#guard_ident), #event_name, guard_result);
    ...
    let completion_state_arms: Vec<TokenStream> = transitions
        ...
        .collect();
    let anonymous_completion_state_arms: Vec<TokenStream> = transitions
        .values()
        .zip(in_states.iter().zip(events.iter().zip(code_blocks.iter())))
        .filter_map(|(event_mappings, (in_state, (events, code_blocks)))| {
            event_mappings
                .values()
                .zip(events.iter().zip(code_blocks.iter()))
                .find(|(mapping, _)| {
                    mapping.event_kind == EventKind::Completion && mapping.event_wildcard
                })
                .map(|(_, (_, code_block))| {
                    quote! {
                        #states_type_name::#in_state => {
    ...
    let new_with_policy_code = match sm.state_data.data_types.get(&starting_state_name) {
        Some(st) if type_matches_state(st, &starting_state_name) => quote! {
    ...
    let state_name_impl = if state_lifetimes.is_empty() {
        quote! {
            impl ::sml::StateName for #states_type_name {
                #[inline(always)]
                /// Returns the generated state variant name.
                fn name(&self) -> &'static str { match self { #(#state_name_arms)* } }
            }
        }
    } else {
        quote! {
            impl<#state_lifetimes> ::sml::StateName for #states_type_name<#state_lifetimes> {
                #[inline(always)]
                /// Returns the generated state variant name.
                fn name(&self) -> &'static str { match self { #(#state_name_arms)* } }
            }
    ...
    let completion_origin_generics =
        sm.completion_generics_with_lifetimes(&completion_lifetimes, completion_generic_type);
    let (
        completion_origin_impl_generics,
        completion_origin_type_generics,
        completion_origin_where_clause,
    ) = completion_origin_generics.split_for_impl();
    let temporary_context_uses_event_generics = sm.temporary_context_uses_event_generics();
    let process_completion_generics = if temporary_context_uses_event_generics {
        sm.event_generics_with_lifetimes(&completion_lifetimes)
    } else {
        completion_origin_generics.clone()
    };
    let (process_completion_impl_generics, _, process_completion_where_clause) =
        process_completion_generics.split_for_impl();
    let temporary_context_generics = if temporary_context_uses_event_generics {
        sm.event_generics.clone()
    } else {
    ...
    let completion_origin_definition = if has_completion_events {
        quote! {
            enum #completion_origin_type_name #completion_origin_impl_generics
                #completion_origin_where_clause
            {
    ...
    let process_completion = if has_completion_events {
        quote! {
            #[inline]
            #is_async fn process_completion #process_completion_impl_generics(
                &mut self,
                #temporary_context
                origin: &#completion_origin_type_name #completion_origin_type_generics
            ) -> Result<bool, #error_type>
                #process_completion_where_clause
            {
                match self.state {
    ...
    let process_anonymous_completion = if has_anonymous_completion {
        quote! {
            #[inline]
            #is_async fn process_anonymous_completion #temporary_context_impl_generics(
                &mut self,
                #temporary_context
            ) -> Result<bool, #error_type>
            ...
            {
                match self.state {
                    #(#anonymous_completion_state_arms,)*
                    _ => Ok(false),
                }
            }
        }
    ...
    let process_exception = if has_exception_handlers {
        quote! {
            #is_async fn process_exception #temporary_context_impl_generics(
                &mut self,
                #temporary_context
                error: #error_type,
    ...
    let unlocked_process = quote! {
        #[inline]
        #is_async fn __sml_process_event_unlocked #process_event_impl_generics (
            &mut self,
            #temporary_context
    ...
    let initialize = {
        let stabilize = if has_anonymous_completion {
            quote! {
                while self.process_anonymous_completion(
    ...
    let machine_trait_impl = if !is_async_state_machine && sm.temporary_context_type.is_none() {
        quote! {
            impl #machine_impl_generics
                ...
                for #state_machine_type_name<#state_lifetimes #context_type_ident, #policy_type_ident>
    ...
    let terminated_trait_impl = quote! {
        ...
        impl<#state_lifetimes #context_type_ident: #state_machine_context_type_name, #policy_type_ident: ::sml::Policies>
            ::sml::Terminated for #state_machine_type_name<#state_lifetimes #context_type_ident, #policy_type_ident>
        {
            #[inline(always)]
    ...
    let default_constructor_impl = quote! {
        #[allow(missing_docs)]
        impl<#state_lifetimes #context_type_ident: #state_machine_context_type_name>
            #state_machine_type_name<#state_lifetimes #context_type_ident, ::sml::NoPolicy>
        {
            #new_sm_code
        }
    };
    let testing_state_data_api =
        sm.state_data
            .data_types
            .get(&starting_state_name)
            .map(|state_data_type| {
                quote! {
                    /// Creates a machine in its initial state with explicit state data.
                    pub fn new_with_state_data(
                        context: #context_type_ident,
                        state_data: #state_data_type,
                    ) -> Self
                    where
                        #policy_type_ident: core::default::Default,
                    {
                        let policy = <#policy_type_ident as core::default::Default>::default();
                        Self::new_with_state_data_and_policy(context, state_data, policy)
                    }

                    /// Creates a machine in its initial state with explicit data and policy.
                    pub fn new_with_state_data_and_policy(
                        context: #context_type_ident,
                        state_data: #state_data_type,
    ...
    let testing_state_api = quote! {
        #[allow(missing_docs)]
        impl<#state_lifetimes #context_type_ident: #state_machine_context_type_name, #policy_type_ident>
                ...
                ::sml::TestingAccess,
        {
            ...
            /// Replaces the current state and returns the previous state.
